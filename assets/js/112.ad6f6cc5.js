...
const dyteChatToggleCss = ":host{position:relative;display:block;font-family:var(--dyte-font-family, sans-serif)}.unread-count{position:absolute;right:var(--dyte-space-3, 12px);-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;--tw-bg-opacity:1;background-color:rgba(var(--dyte-colors-brand-500, 33 96 253) / var(--tw-bg-opacity));font-size:12px;display:flex;height:var(--dyte-space-5, 20px);width:var(--dyte-space-5, 20px);align-items:center;justify-content:center;border-radius:9999px}";
...
const DyteChatToggle = class {
  constructor(hostRef) {
    ...
    this.toggleChat = () => {
      this.chatActive = !(this.states.activeSidebar && this.states.sidebar === 'chat');
      this.unreadMessageCount = 0;
      this.lastMessageCount = this.meeting.chat.messages.length;
  ...
  meetingChanged(meeting) {
    if (meeting != null) {
      this.chatUpdateListener = ({ message }) => {
        if (message != null) {
          const diff = meeting.chat.messages.length - this.lastMessageCount;
          if (!this.chatActive) {
            this.unreadMessageCount = diff;
          }
          else {
            this.lastMessageCount = meeting.chat.messages.length;
          }
        }
      };
  ...
  render() {
    return ((0,_index_da7bf46b_js__WEBPACK_IMPORTED_MODULE_0__.h)(_index_da7bf46b_js__WEBPACK_IMPORTED_MODULE_0__.H, { title: this.t('chat') }, this.unreadMessageCount !== 0 && !this.chatActive && ((0,_index_da7bf46b_js__WEBPACK_IMPORTED_MODULE_0__.h)("div", { class: "unread-count" }, (0,_index_da7bf46b_js__WEBPACK_IMPORTED_MODULE_0__.h)("span", null, this.unreadMessageCount))), (0,_index_da7bf46b_js__WEBPACK_IMPORTED_MODULE_0__.h)("dyte-controlbar-button", { class: { active: this.chatActive }, size: this.size, onClick: this.toggleChat, icon: this.iconPack.chat, label: this.t('chat') })));
  }
